...
public class AuthenticationFilter implements Filter{
	...
	public AuthenticationFilter(AccessAuthorizedNode accessAuthorizedNode){
		...
		this.authorizedTokens = this.accessAuthorizedNode
				.getAccessAuthorizedNodes().stream()
				.map(AuthorizedToken::getToken)
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		//Unauthorized
		if (!validateXAuthToken(request.getHeader(X_AUTH_TOKEN_HEADER))) {
			logger.warn("authentication failed for: {}", req.getServerName());
			response.sendError(401, "authentication failed");
		}
		//unsupported API version
		if (!validateAcceptHeader(request.getHeader(ACCEPT_HEADER))) {
